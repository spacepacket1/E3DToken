/**
 * Check the wrapped E3D token address and balances on Base.
 */
...
async function main(): Promise<void> {
  ...
  try {
    const wrapped = await baseTokenBridge.getWrappedAsset(e3dTokenId);
    wrappedAddress = wrapped.toString();
  } catch {
    console.log("Wrapped E3D not yet attested on Base. Run 'npm run attest' first.");
    return;
  }

  console.log(`Wrapped E3D address on Base: ${wrappedAddress}`);
  ...
  const token = new Contract(wrappedAddress, ERC20_ABI, baseProvider);
  ...
  // Check recipient and treasury balances
  const seen = new Set<string>();
  const addresses = [
    { label: "Recipient", address: config.baseRecipientAddress },
  ...
  for (const { label, address } of addresses) {
    ...
    try {
      ...
      console.log(`${label} (${address}): balance query failed (non-standard ERC20)`);
    }
  }
}
