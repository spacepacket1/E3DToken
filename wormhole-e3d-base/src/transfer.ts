...
async function main(): Promise<void> {
  ...
  // Transfer — recipient on Base is a plain EVM address (no ATA derivation needed)
  ...
  const recipientAddress = toNative(config.baseChain as Chain, config.baseRecipientAddress);

  console.log(`Transferring ${config.transferAmountE3d} E3D to Base address ${config.baseRecipientAddress}...`);
  const transferTxs = ethTokenBridge.transfer(
    senderAddress,
    { chain: config.baseChain as Chain, address: recipientAddress },
    e3dAddress,
    BigInt(amount)
  );
  ...
  if (vaa) {
    ...
    console.log(`Run 'npm run redeem' to mint wrapped E3D on Base.`);
  } else {
  ...
  const recordPath = saveTransferRecord({
    ...
    destinationChain: config.baseChain,
    e3dTokenAddress: config.e3dTokenAddress,
    baseRecipientAddress: config.baseRecipientAddress,
